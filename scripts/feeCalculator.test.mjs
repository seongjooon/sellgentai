/**
 * feeCalculator.ts 회귀 테스트 (이슈 #12: 마진 계산 반올림 오류)
 *
 * 프로젝트에 테스트 러너(vitest 등)가 설정되어 있지 않아, Node.js의
 * --experimental-strip-types 로 TypeScript 소스를 직접 실행하는 assert 기반
 * 스모크 테스트로 작성했다.
 *
 * 실행: node --experimental-strip-types scripts/feeCalculator.test.mjs
 */
...
test('부동소수점 오차가 발생하기 쉬운 수수료율 조합에서도 정수를 반환한다', () => {
  ...
  const cases = [
    { salePrice: 1234, categoryFeeRate: 0.058 },
    { salePrice: 9999, categoryFeeRate: 0.1188 },
    { salePrice: 100001, categoryFeeRate: 0.078 },
    { salePrice: 1, categoryFeeRate: 0.04 },
  ];

  for (const { salePrice, categoryFeeRate } of cases) {
    const result = calculateRocketGrossFees({
      salePrice,
      categoryFeeRate,
      cost: 0,
